...
1. Development board [Shrike-lite](https://github.com/vicharak-in/shrike-lite).
...
### Register Access
Learning to access (read or write) data at different addresses is one of the most important things you'll have to learn to start programming a &micro;C. Let's take an example. Let's say you have an LED connected between GPIO04 and GND pin of the RP2040, this is also how the [LED](https://github.com/vicharak-in/shrike-lite/blob/main/Docs/hardware_overview.rst#user-leds) on Shrike-lite is mounted on the PCB. You can make the LED light up by making GPIO04 produce 3.3V. You will have to figure out which registers need to be changed to achieve this. You can start reading [Section 2.19](https://datasheets.raspberrypi.com/rp2040/rp2040-datasheet.pdf#page=237) of the RP2040 datasheet for this. After setting up most of the registers, what you are left with is actually making the GPIO04 output HIGH (produce 3.3V). This can be achieved by making 8th bit of GPIO04_CTRL register 1. In a C program, this can be achieved by,
```C
*(uint32_t *) (0x40014000 + 0x024) |= 1 << 8; // Set bit 8 of GPIO04_CTRL register
```
Let's break down the line of C code above. The hex number `0x40014000` is the base address of the GPIO peripheral. And, `0x024` is the offset of GPIO04_CTRL register from GPIO base. Thus, `0x40014024` becomes the actual address of GPIO04_CTRL register. To access the value at that address, `0x40014024` needs to be converted into a pointer first. This is achieved by type casting the address value into a pointe, `(uint32_t *) (0x40014000 + 0x024)`. Now, this pointer can be dereferenced to by adding a leading `*` to access it's value, `*(uint32_t *) (0x40014000 + 0x024)`. The remaining operation, `|= 1 << 8`, means to get the existing value of the GPIO04_CTRL register and perform a bitwise OR operation with `1 << 8 = 0b00000000000000000000000100000000`, which sets the 8th bit regardless of its current state.
