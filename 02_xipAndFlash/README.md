...
```
The only major change here compared to last tutorial is that the first 256 Bytes (`.boot2` and `.crc` sections) are now placed in a separate `.boot2` section. However, there are some interesting things happening in that section that needs some explanation. In a linker script, a `.` represents the location counter. As more code/sections are added, the location counter keeps increasing based on the size of each code/section. You can also define symbols (a symbol and a variable are two different things) containing the value of this location counter at specific locations. That's exactly what's happening in case of `_sboot2 = .` and `_eboot2 = .`. These two symbols contain starting and ending addresses of the `.boot2` section. These two symbols are then used to pad the remaining space of first 252 Bytes with zeros in line `. = . + (252 - (_eboot2 - _sboot2));`. And finally, the `.crc` section is appended at the end.
